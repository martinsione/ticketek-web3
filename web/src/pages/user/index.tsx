...
const getUserActivity =  (wallet: Wallet)=>{
  if(wallet){
    return axios.get(`https://api-ropsten.etherscan.io/api?module=account&action=txlist&address=${wallet}&startblock=0&endblock=99999999&page=1&offset=10&sort=asc&apikey=${API_KEY}`).then(res => res.data)
  }
  }
...
function User({users_list}:Iprops) {
  ...
  useEffect(() => {
    console.log("desde useffect", getUserActivity(account).then(res => res.data))
  }, [account])
